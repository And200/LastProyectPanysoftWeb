...
class OrderPlacedResourceIT {
    ...
    void createOrderPlaced() throws Exception {
        ...
        OrderPlacedDTO orderPlacedDTO = orderPlacedMapper.toDto(orderPlaced);
        restOrderPlacedMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(orderPlacedDTO))
            )
    ...
    void putNewOrderPlaced() throws Exception {
        ...
        OrderPlacedDTO orderPlacedDTO = orderPlacedMapper.toDto(updatedOrderPlaced);

        restOrderPlacedMockMvc
            .perform(
                put(ENTITY_API_URL_ID, orderPlacedDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(orderPlacedDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamOrderPlaced() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderPlacedMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(orderPlacedDTO)))
    ...
    void patchNonExistingOrderPlaced() throws Exception {
        ...
        restOrderPlacedMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, orderPlacedDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(orderPlacedDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOrderPlaced() throws Exception {
        ...
        restOrderPlacedMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(orderPlacedDTO))
            )
            .andExpect(status().isMethodNotAllowed());
