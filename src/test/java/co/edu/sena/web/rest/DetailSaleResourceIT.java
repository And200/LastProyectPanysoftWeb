...
class DetailSaleResourceIT {
    ...
    @Mock
    private DetailSaleService detailSaleServiceMock;
    ...
    void createDetailSale() throws Exception {
        ...
        DetailSaleDTO detailSaleDTO = detailSaleMapper.toDto(detailSale);
        restDetailSaleMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(detailSaleDTO)))
    ...
    void getAllDetailSalesWithEagerRelationshipsIsEnabled() throws Exception {
        when(detailSaleServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDetailSaleMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(detailSaleServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void putNewDetailSale() throws Exception {
        ...
        DetailSaleDTO detailSaleDTO = detailSaleMapper.toDto(updatedDetailSale);

        restDetailSaleMockMvc
            .perform(
                put(ENTITY_API_URL_ID, detailSaleDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(detailSaleDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamDetailSale() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDetailSaleMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(detailSaleDTO)))
    ...
    void patchNonExistingDetailSale() throws Exception {
        ...
        restDetailSaleMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, detailSaleDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(detailSaleDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamDetailSale() throws Exception {
        ...
        restDetailSaleMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(detailSaleDTO))
            )
            .andExpect(status().isMethodNotAllowed());
