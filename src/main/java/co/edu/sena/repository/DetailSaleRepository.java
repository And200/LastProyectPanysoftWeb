...
public interface DetailSaleRepository extends JpaRepository<DetailSale, Long> {
    ...
    @Query(
        value = "select distinct detailSale from DetailSale detailSale left join fetch detailSale.product left join fetch detailSale.presentation",
    ...
    @Query(
        "select distinct detailSale from DetailSale detailSale left join fetch detailSale.product left join fetch detailSale.presentation"
    )
    List<DetailSale> findAllWithToOneRelationships();

    @Query(
        "select detailSale from DetailSale detailSale left join fetch detailSale.product left join fetch detailSale.presentation where detailSale.id =:id"
    )
