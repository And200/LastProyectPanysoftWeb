...
export class PurchaseReceiptComponent implements OnInit {
  ...
  protected onSuccess(data: IPurchaseReceipt[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/purchase-receipt'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
